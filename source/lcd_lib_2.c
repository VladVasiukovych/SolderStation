...
void LCD_SendStrFl(char const *str)
{
  char data;
  while ((data = pgm_read_byte(str))){	//pgm_read_byte(address_short) - Read a byte from the program space with a 16-bit (near) address.
	  str++;							//Чтение байта расположенного в памяти программ контроллера по адресу указываемому в скобках.
	  LCD_WriteData(data);
  }
}

/*функция загрузки пользовательского символа*/
